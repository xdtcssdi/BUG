...
class CrossEntry:
    ...
    def forward(self, Y_train, Y_hat):
        m = Y_train.shape[0]
        cost = - Y_train * cp.log(cp.clip(Y_hat, self.epsilon, 1.0-self.epsilon)) - \
               (1 - Y_train) * cp.log(cp.clip(1 - Y_hat, self.epsilon, 1.0-self.epsilon))
        J = cp.mean(cost, axis=0, keepdims=True)
        # if regularization_mode == 'L1':
        #     J += lambd/m * cp.linalg.norm()
        return cp.squeeze(J)
...
class SoftCategoricalCross_entropy:
    ...
    def backward(self, targets, outputs):
        return -targets / cp.clip(outputs, self.epsilon, 1.0-self.epsilon) + \
               (1 - targets) / cp.clip(1 - outputs, self.epsilon, 1.0-self.epsilon)
