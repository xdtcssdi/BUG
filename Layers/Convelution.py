...
class Convolution(Layer):
    ...
    def init_params(self, pre_nc):  # pre_nc 前一个通道数
        ...
        self.W = np.random.randn(*kernel_shape) * 0.01  # W.shape == (f, f ,pre_nc, nc)
    ...
    def forward(self, input):
        ...
        self.Z = np.zeros((n_w, n_h, self.filter_count, input.shape[-1]))
            ...
            for w in range(n_w):
                for h in range(n_h):
                    for nc in range(self.filter_count):
                        ...
                        self.Z[w, h, nc, i] = np.sum(a_slice * self.W[:, :, :, nc] + self.b[:, :, :, nc])
    ...
    def backward(self, dZ):
        ...
        for i in range(m):
            ...
            self.dA[:, :, :, i] = da_pre[self.padding:-self.padding, self.padding:-self.padding, :] if self.padding > 0 else da_pre
        return Activation.get_grad(self.dA, self.Z_pad, self.activation)
